...
class DisplayRegistry(context: Context) {
    ...
    fun resolveDefault(role: DisplayRole): Int? {
        val snapshot = snapshot()
        return when (role) {
            DisplayRole.INTERACTION -> snapshot.interactionCandidates.firstOrNull()
            DisplayRole.PRESENTATION -> snapshot.presentationCandidates.firstOrNull()
        }
    }
}
